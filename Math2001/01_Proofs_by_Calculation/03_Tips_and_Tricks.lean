...
example {z : ℝ} (h1 : z ^ 2 - 2 = 0) : z ^ 4 - z ^ 3 - z ^ 2 + 2 * z + 1 = 3 :=
  calc
    z ^ 4 - z ^ 3 - z ^ 2 + 2 * z + 1 = (z ^ 4 - 4*z^2 +4) + 3*z ^ 2 - z ^ 3 + 2 * z - 3 := by ring
    _ = (z ^ 2 - 2) ^ 2 + 3*z ^ 2 - z ^ 3 + 2 * z - 3 := by ring
    _ = 0^2 + 3*z ^ 2 - z ^ 3 + 2 * z - 3 := by rw[h1]
    _ = 3*z ^ 2 - z ^ 3 + 2 * z - 3 := by ring
    _ = -z*(z^2 - 2) + 3*(z^2 - 2)+3 := by ring
    _ = -z*0 + 3*0 + 3 := by rw[h1]
    _ = 3 := by ring
...
example {x y : ℝ} (h1 : x = 3) (h2 : y = 4 * x - 3) : y = 9 :=
  calc
    y = (4 * x - 3) - (4*x - 3) + y := by ring
    _ = (4*3 - 3) - (4*x - 3) + y := by rw[h1]
    _ = (4*3 - 3) - y + y := by rw[h2]
    _ = 9 := by ring
...
example {t : ℚ} (ht : t ^ 2 - 4 = 0) :
  ...
  calc
    t^4 + 3*t^3 - 3*t^2 - 2*t - 2 = (t^4 - 8*t^2 + 16) + 3*t^3 + 5*t^2 - 2*t - 18 := by ring
    _ = (t^2 - 4)^2 + 3*t^3 + 5*t^2 - 2*t - 18 := by ring
    _ = 0^2 + 3*t^3 + 5*t^2 - 2*t - 18 := by rw[ht]
    _ = 3*t*(t^2 - 4) + 5*t^2 + 10*t - 18 := by ring
    _ = 3*t*0 + 5*t^2 + 10*t - 18 := by rw[ht]
